...
def test_clarification_case_stops_before_model_or_tools() -> None:
    ...
    assert "cung" in result.final_answer.lower() or "email" in result.final_answer.lower()
    assert "tho" in result.final_answer.lower() or "phone" in result.final_answer.lower()
    assert "dia" in result.final_answer.lower() or "giao" in result.final_answer.lower()
...
def test_guardrail_case_refuses_without_tools() -> None:
    ...
    assert result.tool_calls == []
    assert "the" in result.final_answer.lower() or "loi" in result.final_answer.lower() or "xin" in result.final_answer.lower()
    assert any(x in result.final_answer.lower() for x in ["tin", "vi", "ch"])
